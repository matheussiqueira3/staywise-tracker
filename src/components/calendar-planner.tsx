...
export function CalendarPlanner({ trips, rules, onOpen, onSave }: CalendarPlannerProps) {
  ...
  return <div className="planner-shell">
    <div className="planner-toolbar"><div className="planner-region"><span className="planner-label">Planejar em</span><div className="segmented" role="group" aria-label="Região da viagem"><button aria-pressed={region === "brazil"} className={(region === "brazil" ? "selected " : "") + "region-option brazil"} onClick={() => chooseRegion("brazil", "Brazil")}>Brasil</button><button aria-pressed={region === "schengen"} className={(region === "schengen" ? "selected " : "") + "region-option schengen"} onClick={() => chooseRegion("schengen", "Italy")}>Schengen</button></div></div><div className="year-switcher" aria-label="Escolher ano"><button className="circle-button" aria-label="Ano anterior" onClick={() => setYear((value) => value - 1)}>‹</button><strong>{year}</strong><button className="circle-button" aria-label="Próximo ano" onClick={() => setYear((value) => value + 1)}>›</button></div><button className={"button " + (planning ? "secondary" : "primary") + " planner-clear"} onClick={() => planning ? clearSelection() : startPlanning()}>{planning ? (start && end ? "Limpar seleção" : "Cancelar") : "Planejar viagem"}</button></div>
    <div className="calculation-strip"><div><span className="planner-label">Janela móvel · {region === "brazil" ? "Brasil" : "Schengen"}</span><strong>{activeRule.windowDays} dias</strong><small>{formatDate(windowStart, { day: "2-digit", month: "short", year: "numeric" })} — {formatDate(windowEnd, { day: "2-digit", month: "short", year: "numeric" })}</small></div><div className={"calculation-score " + status.status}><strong>{status.used}<small> / {activeRule.limit}</small></strong><span>{status.remaining > 0 ? status.remaining + " dias restantes" : status.status === "over" ? "limite ultrapassado" : "limite atingido"}</span><small>dias usados na janela</small></div></div>
...
function MonthCalendar({ year, month, today, trips, selectedRegion, selectedStart, selectedEnd, isSelected, isInWindow, tripForDay, onDay }: { year: number; month: number; today: string; trips: Trip[]; selectedRegion: Region; selectedStart: string | null; selectedEnd: string | null; isSelected: (date: string) => boolean; isInWindow: (date: string) => boolean; tripForDay: (date: string) => Trip | undefined; onDay: (date: string, trip?: Trip) => void }) {
