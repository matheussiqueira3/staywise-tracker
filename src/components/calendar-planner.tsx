...
export function CalendarPlanner({ trips, rules, onOpen, onSave }: CalendarPlannerProps) {
  ...
  const displayStart = addDays(anchorDate, -359);
  const rollingMonths = monthsInRange(displayStart, windowEnd);
  ...
  return <div className="planner-shell">
    <div className="planner-toolbar"><div className="planner-region"><span className="planner-label">Planejar em</span><div className="segmented" role="group" aria-label="Região da viagem"><button aria-pressed={region === "brazil"} className={(region === "brazil" ? "selected " : "") + "region-option brazil"} onClick={() => chooseRegion("brazil", "Brazil")}>Brasil</button><button aria-pressed={region === "schengen"} className={(region === "schengen" ? "selected " : "") + "region-option schengen"} onClick={() => chooseRegion("schengen", "Italy")}>Schengen</button></div></div><div className="rolling-display" aria-label="Display rolling de 360 dias"><strong>Rolling 360 dias</strong><small>{formatDate(displayStart, { day: "2-digit", month: "short" })} — {formatDate(windowEnd, { day: "2-digit", month: "short", year: "numeric" })}</small></div><button className={"button " + (planning ? "secondary" : "primary") + " planner-clear"} onClick={() => planning ? clearSelection() : startPlanning()}>{planning ? (start && end ? "Limpar seleção" : "Cancelar") : "Planejar viagem"}</button></div>
